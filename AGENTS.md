...
Runtime Python, pacote na versão 0.0.27. Wave 10.2 (#87/#89/#90) entregue; a run autônoma #108 continua o backlog de segurança, ferramentas e workflows. A contabilização inclui usage recebido após o fechamento funcional, reconciliado ao término dos workers sob o fence da aquisição (#111/#112). Resultados, cache e atribuição por nó conservam o histórico do fechamento; perdas antes da persistência continuam no épico #10. As correções posteriores a 0.0.27 na main ainda não foram publicadas no PyPI. Cada PR integrada exige CI Python 3.11/3.13 e revisão independente do SHA final. #86 aguarda volume de insights; #70 aguarda reprodução da falha histórica. Estado, evidências e próximos trabalhos estão em `docs/STATUS.md` e no objetivo #108.
...
- **Agentes implementadores** (worktrees) não tocam `CHANGELOG.md`, `CLAUDE.md`, `AGENTS.md`, `docs/STATUS.md`, versão no `pyproject.toml`; o coordenador prepara um commit de fechamento na integração e o inclui na última PR da rodada. Manter `AGENTS.md` como espelho de `CLAUDE.md`. O teste `test_skill_file_stays_within_the_line_budget` cobre todos os `SKILL.md` builtin com teto de 800 linhas (`workflow-authoring` está em 799). A skill `use-lohra` tem 3 cópias (export, `docs/skills`, `.codex/skills` local gitignored) com teste anti-drift — sincronizar a `.codex` à mão após mudar.
