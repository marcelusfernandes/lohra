...
def bind_workflow_notifier(
    ...
    - **durable notice** (SUP-05): when ``db`` is given, EVERY non-cancelled
      ...
      7 days, fingerprint dedup, per-owner cap, all store-enforced. Delivery
      across processes is at-least-once by design: a crash between publish and
      consumer ack can deliver the notice again; dedup absorbs an identical
      republication, and a repeated delivery of an already-acked one is the
      accepted cost of not losing the fact. The claim/ack lifecycle is the
      consumer's job (session turn injection), not this notifier's. When the
      durable channel is bound, the live inbox is NOT written — the durable
      notice already reaches the owning session on its next turn, and a second
      channel would deliver the same fact twice.
    - **live inbox fallback** (backward compatibility only): without ``db``,
      ``resolve_inbox(session_id)`` returns anything with ``enqueue_steer`` (a
      GatewaySession), or None, and the summary lands as a system-reminder in
      the tail of the next loop iteration. Process-local and at-least-once only
      within the process; with no durable store this remains the only path.
    ...
    owner never publishes a summary over the recovering owner's run. The
    OWNER arrives WITH the callback — captured from the RunState at the
    accepted terminal write — never re-derived via a late ``run_owner``
    lookup, which a straggler could answer with the wrong session.

    A run nobody owns publishes NOTHING, durable or live; a session that has
    gone away from the live resolver is a silent no-op on the fallback. The
    notification is a courtesy, and the rollup is always there to poll.
    """

    def on_run_done(owner: str | None, run_id: str, _status: str, summary: str) -> None:
        session_id = (owner or "").strip()
        if not session_id:
        ...
        if db is not None:
            try:
                ...
                logger.exception("workflow: durable notice publish failed for run %s", run_id)
            return
        # No durable store: the live inbox is the only delivery path left.
        try:
            inbox = resolve_inbox(session_id) if resolve_inbox is not None else None
            if inbox is not None:
                inbox.enqueue_steer(summary)
        except Exception:
            logger.exception("workflow: live inbox steer failed for run %s", run_id)
