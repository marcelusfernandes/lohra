...
description: Choose, size and author a dynamic workflow spec for run_workflow, and read its rollup honestly. Load this before authoring a workflow or interpreting its rollup. Do not load for single-step tasks, file edits or questions answered by a direct tool call when no workflow is needed.
version: 1.0.0
---
...
**Granularity is the cost model.** An `agent` is one cell; a `pipeline` has one per `(item, stage)`. Each aggregate node (`parallel`, `verify`, `judge_panel`, `loop_until_dry`, `gate`, `completeness_check`) is one cache unit and re-executes whole when incomplete or changed. A nested workflow node is deterministic control, not a parent cache cell; internal completed cells share the run cache, keyed by nested spec identity plus the structured calling-node scope. Legacy cells replay only with recorded scope or explicit harness artifact owners; otherwise `legacy_scope_unproven` means revalidation with unknown attributable cost, never a free replay. Changing `ref` reruns control and lets child cells hit/miss independently. Bumping `meta.version` rekeys that spec and defeats a surgical pivot.
...
- **`paused` is not failure**, and `reason` tells you which kind it is. Either way the finished nodes are kept.
  ...
  - **`route_fault`** — a ROUTE is **dead**: a refused credential (`auth_failed`), or a `retries` series you
    ...
    COMMAND on the **same** `run_id` — no re-authored spec: `checkpoint_answers={"<route.node_id>": {"provider": "…", "model": "…", "effort": "…"}}` re-routes that ONE node in the spec on file (cached cells replay, so only the dead node is re-paid); `{"<route.node_id>": "abort"}` cancels the run (`cancelled`, never `failed`). The acceptance echoes `rerouted {node_id, from, to}` — read it. Where the node routes by `tier`, answer provider AND model (a model alone keeps the tier's provider and dies again). Pick the new route yourself **only** on the same provider and credential/billing route and never costlier; else the human decides. Refused didactically (run stays paused, costs nothing): another node, a non-route field, the route that just died (an `effort` change does not lift that on a refused credential), a nested template's route, a rigor node that declared no route of its own, an answer plus an explicit spec (one channel per resume), and any route answer — `abort` included — sent to a run that is not paused on a route. **Check first whether the node was already re-routed for you:** if the OPERATOR wrote `~/.lohra/workflow_routes.json`, a dead route may have been moved automatically to a cheaper route on their list — the run's faults say `re-routed by operator envelope: <from> -> <to>` and the audit carries `node.rerouted`. That is configuration, not your decision and not the spec's: you cannot create, widen or read that envelope, and when it refuses (unknown price, costlier, provider not enabled, allowance spent) the run pauses here as usual — the pause's hint names which of those it was.
