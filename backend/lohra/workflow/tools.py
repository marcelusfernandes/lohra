...
RUN_GUIDANCE = (
    ...
    "Answer with the key the pause gave you — it differs on purpose from the "
    "'sub[<ref>]:<node>' that faults, costs and cache_preview report, which "
    "names the TEMPLATE.\n"
    ...
    "reported as 'sub[<ref>]:<node>'; 'unknown' entries (anything downstream of "
    "a miss, a nested ref that will not load) are nodes the preview cannot "
    "recompute, never free replays. Once it is running, workflow_status reports "
