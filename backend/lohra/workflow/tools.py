...
RUN_GUIDANCE = (
    ...
    "finish before anything downstream runs. Optional 'retries' (default 0) "
    "re-spawns a DEAD branch fresh, up to that many times, before giving up on it.\n"
    ...
    "{complete, missing} — pair it with loop_until_dry to keep digging, or mark "
    ...
    "or 'pause' (ask the same question again). A 'default' auto-answers the gate "
    ...
    "'branches'/'stages', a routing knob used to be silently ignored (no "
    "error, no fault, the session's own model billed) and is now REFUSED at "
    "validation (issue #82) — the leaves of those shapes run on the node's "
    "route.\n"
    ...
    "allow-list; a 'model' the provider does not have runs ONE leaf on the model "
    "the operator mapped for that node's 'tier' and reports the swap as an "
    "advisory fault you must relay, and where no such model exists it PAUSES the "
    "run (reason 'route_fault') for a human instead of degrading node by node. Nodes in the SAME DAG may name DIFFERENT providers, including "
    ...
    "A non-quota provider death already burned that node's 'retries' on the SAME route before it nulled ONLY IF the author WROTE the field: the default covers an empty answer alone, while a written 'retries' also covers a generic provider death — never a quota pause, a refused credential (auth_failed), either timeout, or a cancel. Where the field was written your resume is the second attempt at the SHAPE, not the first at the route; where it was not, the route has been tried exactly once.\n"
    "On a "
    ...
    "A 'route_fault' pause means a ROUTE is DEAD, not that a call was unlucky: a "
    ...
    "Before you diagnose a node's route, check whether the OPERATOR's route "
    "envelope already re-routed it: read the run's faults (and the audit's "
    "'node.rerouted') — the node may be running on a route the operator "
    "pre-authorized before the run, not on the one its spec names.\n"
    ...
    "intentional before re-paying tokens_to_repay; the counts are CELLS, so a "
    "pipeline entry carries 'cells'/'stages' and a nested template's nodes are "
    "reported as 'sub[<call>]:<node>' with template metadata; 'unknown' entries (anything downstream of "
    "a miss, a nested ref that will not load) are nodes the preview cannot "
    "recompute, never free replays. legacy_scope_unproven means an old cell "
    "cannot be attributed to this call; its cost is unknown, and checkpoints "
    "need a new answer. Once it is running, workflow_status reports "
    "cells_replayed/tokens_saved and marks each replayed node in progress, so "
    ...
    "spend, reported back as {total, spent, remaining, overrun, overrun_max}) is "
    "a human decision, never an agent one. 'overrun' above 0 means a leaf "
    "ALREADY IN FLIGHT crossed the ceiling in force and was charged (never "
    "killed mid-call); 'overrun_max' is how far over the run has EVER been, "
    "which a raised ceiling does not erase. Either way the run says so with an "
    "advisory fault and is not degraded by it.\n"
...
_STATUS_SCHEMA = {
    "description": (
        ...
        "status 'degraded' means at least one node ended null or an UNRECOVERED "
        "fault exists: read 'faults' (and 'faults_total' on a resumed run) before "
        "trusting 'outputs'; say which parts are missing instead of writing around "
        "holes. A fault also listed under 'recovered_faults' was fixed by a "
        "same-route re-spawn — the node produced its output, so it does not count "
        "against the verdict and 'leaf_respawns' says what it cost. "
        "A fault listed under 'advisory_faults' is a warning about a node that "
        "CONCLUDED — a leaf that miscounted the sha256/bytes of a file it really "
        "wrote, or a cell replayed under an operator policy or harness version "
        "other than the one it was stored under — so the harness measurement "
        "stands and the verdict ignores it; relay it, do not re-run the node "
        "over it. "
