...
- **A `checkpoint` inside a nested template is a DIFFERENT question** (issue #78, 2026-09-05). Node ids are per spec, so a parent and a template it references may both call a gate `cp` without knowing about each other — and until this they shared one answer, because `checkpoint_answers` reached the nested engine keyed by the bare id. With `accept` in play (above) the collision approved work nobody was asked about: one `"sim"` for the parent's *"ok to start?"* also opened the template's *"delete prod?"*, and the pause latch being first-wins meant that second question was never shown to the human who supposedly answered it. **A nested gate is now asked and answered under `sub[<ref>]:<node id>`** — the namespace `fold_nested` already gives a nested run's faults, costs, route faults and `required` failures — and its pause payload carries that `node_id` plus `template: <ref>`, the way a nested `route_fault` already names the template the remedy lives in. **The answer by command uses that key**: `run_workflow(resume_run_id=…, checkpoint_answers={"sub[<ref>]:<id>": "<answer>"}`) — the pause says which key, and the bare id no longer reaches a nested gate (it matches no pending checkpoint, so the launch is refused didactically, naming the right one; the run stays paused and costs nothing). Namespaced are the ANSWER and the PAYLOAD only: the cache CELL was never ambiguous — `cell_hash` is namespaced by the child's own `spec_identity`, so a parent's cached answer cannot replay as the child's — and the pause latch and its fault text keep the bare id, which `fold_nested` prefixes on the way up (spelling it in both places would print the namespace twice). Depth is capped at 1, so one prefix level is the whole ladder. The prefix now has ONE builder (`lohra/workflow/namespacing.py`, pinned by a source-level test): the answer key is the site where one character of drift silently approves the wrong question.
...
1. an answer that reads as a route answer — a routing object **or the word `abort`** — keyed to a node whose TYPE takes routing, on a run not paused at `route_fault`. Gated on "does this type take routing" rather than on "is it a checkpoint", because a checkpoint one level down is answered under a namespaced key (`sub[ref]:node`, #78 — the same shape a nested *route* already had) that names no node of the parent spec, so its answers reach the decision with no type that spec can name — a human answering such a gate `"abort"` must not have it refused as a misplaced route;
