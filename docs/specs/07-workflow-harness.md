...
- **A `checkpoint` inside a nested template is a DIFFERENT question** (issue #78, 2026-09-05). Node ids are per spec, so a parent and a template it references may both call a gate `cp` without knowing about each other — and until this they shared one answer, because `checkpoint_answers` reached the nested engine keyed by the bare id. With `accept` in play (above) the collision approved work nobody was asked about: one `"sim"` for the parent's *"ok to start?"* also opened the template's *"delete prod?"*, and the pause latch being first-wins meant that second question was never shown to the human who supposedly answered it. **A nested gate is now asked and answered under `sub[<workflow node id>]:<node id>`** — namespaced by the CALL — and its pause payload carries that `node_id` plus `template: <ref>`, the way a nested `route_fault` already names the template the remedy lives in. **The answer by command uses that key**: `run_workflow(resume_run_id=…, checkpoint_answers={"sub[<node>]:<id>": "<answer>"}`) — the pause says which key, and the bare id no longer reaches a nested gate (it matches no pending checkpoint, so the launch is refused didactically, naming the right one; the run stays paused and costs nothing; an *extra* unmatched key still rides along unread — the refusal is about the pending key being absent, never about an unknown key being present). **The key is the calling NODE, not the template ref**, and that is the one place the harness deliberately namespaces by caller instead of callee: the first cut of this slice keyed by `<ref>`, and the adversarial review showed two `workflow` nodes running ONE template with different args (`target: staging` beside `target: PROD`) then share a single answer — approving staging approved PROD, and *"delete PROD?"* was never shown. Node ids are unique inside a spec by validation; template refs are not unique inside a spec at all. Everything a ROLLUP reports (faults, costs, route faults, `required_failure`, `cache_preview` rows) keeps the `sub[<ref>]:` namespace it has had since #61 — a reader diagnosing a fault wants the template, a human answering wants the call — so `cache_preview` threads the two prefixes separately down `_walk`. Namespaced are the ANSWER and the PAYLOAD only. **The cache CELL is a separate axis**: `cell_hash` is prefixed by the running spec's `spec_identity`, which separates parent from template *whenever their `(meta.name, meta.version)` differ* — which is every ordinary template, but is not guaranteed. A template whose identity equals its caller's, asking a byte-identical question under the same node id, produces an equal hash: the nested gate HITS the parent's cached answer and replays it before the key is consulted. Known limitation, pinned by test rather than papered over (`test_a_template_sharing_the_parents_spec_identity_replays_its_cell`); the remedy — namespacing the cell too, or refusing the identity collision at validation — moves every cached cell of every nested run and is a slice of its own. The pause latch and its fault text keep the bare id, which `fold_nested` prefixes on the way up (spelling it in both places would print the namespace twice). Depth is capped at 1, so one prefix level is the whole ladder. The prefix now has ONE builder (`lohra/workflow/namespacing.py`, pinned by a source-level test that parses the package rather than grepping it): the answer key is the site where one character of drift silently approves the wrong question.
...
1. an answer that reads as a route answer — a routing object **or the word `abort`** — keyed to a node whose TYPE takes routing, on a run not paused at `route_fault`. Gated on "does this type take routing" rather than on "is it a checkpoint", because a checkpoint one level down is answered under a namespaced key (`sub[<workflow node>]:<id>`, #78 — the shape a nested *route* already had, though keyed by the call rather than by the template) that names no node of the parent spec, so its answers reach the decision with no type that spec can name — a human answering such a gate `"abort"` must not have it refused as a misplaced route;
