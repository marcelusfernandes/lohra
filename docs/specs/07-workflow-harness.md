...
- **What is still flying at the seal has a house.** `_seal` gives each deferred leaf one last non-blocking chance and then closes the books: a leaf that landed in the meantime is accounted for real, and one still inside a provider call becomes **one more `usage_uncertain_leaves` plus the fault `leaf still running at seal; provider usage unknown`** — never a 0 reported as a fact. The count and the seal happen in the same critical section, so a hook firing one instant later can neither add usage the persisted rollup no longer contains nor contradict the fault already written about that leaf.
