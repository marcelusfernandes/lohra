...
1. an answer that reads as a route answer — a routing object **or the word `abort`** — keyed to a node whose TYPE takes routing, on a run not paused at `route_fault`. Gated on "does this type take routing" rather than on "is it a checkpoint", because a checkpoint one level down is answered under a namespaced key (`sub[<workflow node>]:<id>`, #78 — the shape a nested *route* already had, keyed by the call) that names no node of the parent spec, so its answers reach the decision with no type that spec can name — a human answering such a gate `"abort"` must not have it refused as a misplaced route;
