...
**Fail-soft on the FILE, fail-closed on the ENTRY.** An absent, unreadable or malformed file means "no envelope" and never an exception — and here the fail-soft direction *is* the fail-closed one, since no envelope is exactly today's pause. But an entry that declares a knob this version does not **enforce** — `max_usd_per_cell` and `on`, both sketched in the issue — is **dropped whole**. Both would *narrow* the authorization, so honouring the list without them would silently widen what the operator wrote. A typo'd `max_fallbacks_per_run` falls back to the default, never to "unlimited".
