...
1. a route-shaped answer keyed to a **non-`checkpoint` node on a run not paused at `route_fault`** — caching it as that node's output would answer a question nobody asked and poison the cell;
