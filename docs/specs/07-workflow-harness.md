...
**Reused as-is:** `make_child_factory` base isolation, `subagent_dispatch` auto-deny + `_CHILD_EXCLUDED_TOOLS`, `validate_public_url` SSRF guard (after the per-hop operator host gate, §8.3), `compose_dispatch` intercept pattern, `tool_result`/`tool_error` envelope, SessionDB lineage + `compression_locks`, `run_conversation` kernel + steer-into-tail, the core's capped pool + queue-when-over-cap + terminal-only eviction + `cancel`/`shutdown`.
