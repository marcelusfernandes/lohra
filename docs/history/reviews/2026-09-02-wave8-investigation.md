...
## Wave 8.2 — Q2, #43-C, #45-E4/E5, #42-E3 (0.0.21, 2026-09-03)
Q2 `leaf_respawns` (run recuperado não sela `degraded`; desconto por identidade e multiset em `derive_status` e `carried_faults`, `prior_recovered` durável) · pausa `route_fault` (gatilho estreito; faults da série e irmãs do mesmo nó pause-caused; `node_id` autorado no payload, `template` no nested; hint SUP-04 concatenado sob teto do operador) · manifesto de artefato medido pelo harness (escopo `runs/<run_id>/` + `fs_allow` com realpath re-check; colunas laterais; `artifact_changed`) · interrupt aborta o stream (fechamento real provado contra servidor SSE local nos dois SDKs; `usage_uncertain` cross-estirão). Reviews adversariais em `route_fault` (3 médios + fan-out) e stream-abort (2 médios) — todos os gates aplicados. **Dogfood T9 via Codex PASS**: rota morta + `retries: 1` → `paused/route_fault`, `watch` sai, resume adaptado no mesmo run → `cache_preview {replay: 1, never_completed: 1}` → `complete` com replay. Q2 e stream-abort não são disparáveis ao vivo de forma determinística; a evidência são os discriminadores (o do stream-abort provado por sabotagem e contra socket). Evidência: `docs/history/evidence/wave8.2-dogfood/`.
