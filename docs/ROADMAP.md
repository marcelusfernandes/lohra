...
Branches: `feat/phase-8-workflow-harness` (A–F) e `feat/phase-8-pipeline-resume` (D+G) — ambas mergeadas; Fase 8 completa (A–J + polish).
...
- [x] **Taint real** ✅ — `agent/taint.py`: `TaintTracker` (sticky/sessão) + `taint_wrap` no dispatch; turno que ingere web/MCP → `run_workflow` roda leaves sem fs+egress. §8.2 (controle 3), discriminador no caminho real.
