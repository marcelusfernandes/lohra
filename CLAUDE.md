...
Runtime Python, pacote na versão 0.0.27. Wave 10.2 (#87/#89/#90) entregue; a run autônoma #108 continua o backlog com checkpoints estruturados (#106), search opt-in e fetch com policy por redirect (#55/#56), ordem de acesso a arquivos no mesmo lote (#97), credenciais de assinatura verificadas por requisição (#9), conexão de fetch vinculada a IPs públicos validados (#13), aprovações isoladas por consumidor (#129), aceite de steer com callbacks fora do lock (#69), cancelamento durável por aquisição (#126), criação de sub-sessão condicionada ao aceite do executor (#136), submissão de workflow publicada antes de executar (#138), auto-resume armado após encerramento da execução anterior (#127) e contabilização de pipeline após timeout (#111). As correções da main posteriores a 0.0.27 ainda não foram publicadas no PyPI. Cada PR integrada tem CI Python 3.11/3.13 e revisão independente do SHA final. #86 aguarda volume de insights; #70 aguarda reprodução da falha histórica. Estado, evidências e próximos trabalhos estão em `docs/STATUS.md` e no objetivo #108.
